...
export default function main() {
  ...
  group("page_1 - https://stylishop.com/ae/en/", function () {
    ...
    check(response, {
      "status equals 200": response => response.status.toString() === "200",
      "$.status contains false": response =>
        jsonpath
          .query(response.json(), "$.status")
          .some(values => values.includes("false")),
      "$.statusMsg equals No store credit wallet found for customer!":
        response =>
          jsonpath
            .query(response.json(), "$.statusMsg")
    ...
    check(response, {
      ...
      "$.response.autoCouponApplied equals B1G1 auto applied": response =>
        jsonpath
          .query(response.json(), "$.response.autoCouponApplied")
          .some(value => value === "B1G1 auto applied"),
      "$.response.autoCouponDiscount equals 262.19": response =>
        jsonpath
          .query(response.json(), "$.response.autoCouponDiscount")
          .some(value => value === "262.19"),
    });
